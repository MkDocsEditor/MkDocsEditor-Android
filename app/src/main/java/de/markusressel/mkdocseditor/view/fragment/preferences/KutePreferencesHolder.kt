...
class KutePreferencesHolder @Inject constructor(private val context: Context, private val iconHelper: IconHandler, private val dataProvider: KutePreferenceDataProvider) {
    ...
    val connectionUriPreference by lazy {
        KuteTextPreference(key = R.string.connection_host_key, icon = iconHelper.getPreferenceIcon(MaterialDesignIconic.Icon.gmi_battery), title = context.getString(R.string.connection_host_title), defaultValue = "127.0.0.1", dataProvider = dataProvider)

    }

    val basicAuthUserPreference by lazy {
        KuteTextPreference(key = R.string.connection_basic_auth_user_key, title = context.getString(R.string.connection_basic_auth_user_title), defaultValue = "", dataProvider = dataProvider)

    }

    val basicAuthPasswordPreference by lazy {
        KutePasswordPreference(key = R.string.connection_basic_auth_password_key, title = context.getString(R.string.connection_basic_auth_password_title), defaultValue = "", dataProvider = dataProvider)
